...
- "CMOS dynamic power is approximately proportional to $C V_{DD}^2 f$, so voltage reduction is commonly treated as a mechanism for reducing dynamic energy when performance constraints permit it [26], [27]." Explanation: the source matrix supports DVS/DVFS generally; added explicit circuit/power-model references for the $C V_{DD}^2 f$ relationship.
