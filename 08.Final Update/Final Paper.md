...
The full source matrix has been moved to an appendix and is available at [Appendix: Source Matrix](Appendix_Source_Matrix.md). The main text retains a concise literature synthesis and the References section; see the appendix for claim-to-source provenance and DOI/URL details.

### Implications for the STM32MP1/STPMIC1 Paper

The presented measurements demonstrate that STM32MP1/STPMIC1-class hardware provides a software-accessible path to configure PMIC rails and observe rail-level voltage changes. However, oscilloscope voltage traces alone do not establish system-level energy savings; claims about energy or power require measured current/power, workload characterization, and timing/transition analysis. See the appendix for the full source matrix, extended caveats, and recommended additional measurements.
